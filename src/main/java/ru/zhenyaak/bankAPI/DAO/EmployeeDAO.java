    ...
    public Card changeCardStatus(Card card) {
        ...
        try (Connection connection = dataSource.getConnection()){
            try (PreparedStatement preparedStatement = connection.prepareStatement("SELECT * FROM Cards WHERE number = ?")){
                ...
                try (ResultSet rs = preparedStatement.executeQuery()){
                    rs.next();
                    cardForUpdate.setId(rs.getInt("id"));
                    cardForUpdate.setNumber(rs.getString("number"));
                    cardForUpdate.setId_account(rs.getInt("id_account"));
                    cardForUpdate.setStatus_card(rs.getString("status_card"));
                }
            } catch (SQLException throwables) {
                throwables.printStackTrace();
                throw new CardNotFoundException("Card with number = " + card.getNumber() + " not found. More details: " + throwables);
            }

            if (card.getStatus_card().equals(cardForUpdate.getStatus_card()))
                throw new CardChangeStatusException("Card with number = " + card.getNumber() + " already has this status");
